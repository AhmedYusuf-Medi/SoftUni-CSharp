    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < numbers.Count; i++)
            {
                ...
                int numIndex = CalculateIndex(currentNumber);

                char currentChar = GetCharFromMessage(numIndex, message);
                Console.Write(currentChar);

                int messegeIndex = CalculateRealIndex(numIndex, message);
                string newMessage = message.Remove(messegeIndex, 1);
                message = newMessage;
            }
